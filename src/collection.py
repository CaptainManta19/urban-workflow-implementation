...
def load_source_manifest(
    ...
    if not isinstance(raw_manifest, dict):
        raise ValueError("Source manifest must be a JSON object with a 'sources' list.")

    manifest_sources = raw_manifest.get("sources", [])
    if not isinstance(manifest_sources, list):
        raise ValueError("The manifest 'sources' field must be a list.")
...
def save_collection_report(report: CollectionReport) -> Path:
    output_path = Path("outputs/reports/collection_report.json")
...
def collect_sources_from_project_root(project_root: Path) -> tuple[list[CollectedSource], CollectionReport]:
    ...
    try:
        manifest_specs, manifest_skips, manifest_path, manifest_source_count = load_source_manifest(
            project_root
        )
        ...
        skipped_items.append(
            SkippedCollectionItem(
                item_label="data/source_manifest.json",
                item_origin="manifest",
                reason=f"Manifest could not be loaded: {exc}",
            )
        )
        manifest_specs = []
        manifest_path = project_root / "data" / "source_manifest.json"
